 ...
 * @return array
 */
function simp_body_classes($classes)
{
...
add_filter('body_class', 'simp_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function simp_pingback_header()
{
	if (is_singular() && pings_open()) {
		printf('<link rel="pingback" href="%s">', esc_url(get_bloginfo('pingback_url')));
	}
}
add_action('wp_head', 'simp_pingback_header');
...
if (!function_exists('simp_post_thumbnail')) :
	/**
...
if (!function_exists('simp_post_thumbnail_full')) :
	/**
	 * Displays an optional post thumbnail.
	 *
	 * Wraps the post thumbnail in an anchor element on index views, or a div
	 * element when on single views.
	 */
	function simp_post_thumbnail_full()
	{
		if (post_password_required() || is_attachment() || !has_post_thumbnail()) {
			echo get_stylesheet_directory_uri() . '/assets/svg/placeholder.svg';
		} else {
...
function simp_pagination()
{
	...
	echo '<div class="simp-pagination"><ul>' . "\n";
